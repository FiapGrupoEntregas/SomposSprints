...
A fazenda (`farm_id`), o **dia** (`selected_day_date`), o **cenário** (`risk_scenario_heavy_rain`)
e o opt-in MLP (`risk_experimental_mlp_enabled`) ficam no `st.session_state`, em chaves próprias
(`components/state.py`) — **nunca** na chave de um
...
- Todo acesso passa por `services/api_client.py`: `list_farms()` e `get_farm(id)` (cache de 10 min),
  `get_terrain(id)` (cache de 24 h), `get_risk(id, days, scenario, include_experimental_mlp)`
  (cache de 1 h separado pela opção experimental) e `get_device_limit(...)`
...
O smoke test local da aplicação e os resultados da última execução da suíte estão em
