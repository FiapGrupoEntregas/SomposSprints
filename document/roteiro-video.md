...
| 9 | 3:55–4:22 | D | (a) Terminal com a contagem da tabela `policy`; (b) resposta do `GET /api/v1/audit` no `/docs`, com uma linha de `decision_log`; (c) saída do `pytest` | Do lado dos dados, carregamos **1.525.473 apólices reais** do seguro rural (PSR/SISSER, dados abertos). Nomes e documentos são descartados na leitura, mas o `proposal_id` permanece e permite vínculo com o CSV público, que contém o nome — a limitação está documentada na I5. **Toda decisão** fica registrada com entrada, saída e a versão da regra e do modelo que decidiram, e a escrita exige chave de API. São **_(recontar na véspera)_** testes automatizados, todos offline. |
