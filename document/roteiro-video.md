...
| 4 | 1:25–1:48 | B | Mesma página, rolar até o **cartão "🤖 Probabilidade de sinistro (modelo)"**, ao lado do nível por regras. Mostrar a linha da AUC-PR e a ressalva que o cartão exibe por inteiro | Ao lado da regra, um **modelo treinado com sinistros reais** do seguro rural dá uma segunda leitura para a seguradora. A parte honesta está na tela: **o modelo não superou o baseline por regras**, e a amostra é pequena demais para provar superioridade de qualquer lado. O alerta ao operador continua vindo da **regra explicável**. |
...
- **Suavizar o resultado do modelo.** Não diga "o modelo ficou perto", "praticamente empatou" nem
  "é promissor". A frase correta está no bloco 4 e em [dados-e-modelo.md](dados-e-modelo.md): ele
  **não superou** o baseline, e a amostra não permitiria demonstrar superioridade nem se ela
  existisse. Isso é resultado publicado, não defeito escondido.
...
| AUC-PR do modelo × baseline | cartão do modelo no front, que lê `api/app/data/model/risk_model_v1.json` em tempo de execução | **o modelo será retreinado antes da entrega.** Não decore: leia o que estiver no cartão na hora da gravação. A frase "não superou o baseline" também sai do artefato — se um retreino inverter o resultado, o cartão inverte a frase sozinho, e a narração tem de acompanhar |
...
- [ ] **Se o modelo tiver sido retreinado**, reabrir o cartão do modelo e conferir a métrica e a
      frase de comparação com o baseline antes de gravar o bloco 4
